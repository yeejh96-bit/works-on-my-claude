...
### 할 일
이 저장소는 Claude Code 플러그인 womc 자체다. 새·기존 프로젝트에 얇은 기본 하네스를 깔고, 사용자가 준 최소 정보에 맞춰 하네스를 스스로 다듬게 한다. 코딩을 모르는 사용자도 Claude Code 가 꾸준히 잘 일하게 만드는 것이 목표.

### 지킬 것
- 철학: 하네스는 얇게, 나머지는 모델을 믿고 맡긴다. 사용자의 승인·결정은 최소로. 하네스 기능은 모델 약점을 덮는 임시 장치라 모델이 좋아지면 뺀다.
