...
`/womc` 한 번이면 — 불변 작업 규칙(CLAUDE.md) · 프로젝트 명세(SPEC.md) · 조사 위임(explore) ·
한 번에 하나씩 구현하는 운영 절차(plan-feature 스킬) 가 한 묶음으로 세팅된다. 스택도 MCP도 강제하지 않는다.
...
| **권한·안전** | `.claude/settings.json` | 항상(읽기전용만 허용) |
