...
- **CLAUDE.md의 "설명 방식" 절 (`womc:brevity`)** — 대화 답변을 명사형·불릿 위주로 짧게 만들어 출력 토큰을 아낀다. 강도 4단계(`케이브맨`·`약하게`·`보통`·`최소`, 기본 `케이브맨`)는 절 안의 표식 값을 고치면 바뀐다. `케이브맨`은 원시인처럼 한 줄에 한 뜻, 2~5 단어 단문으로 말하는 가장 센 단계다. 파일 경로·`경로:줄번호`·다음 할 일·되묻는 질문·위험 경고는 어느 강도에서도 줄이지 않고, SPEC·PLAN·TASKS 같은 파일 문서에는 적용하지 않는다.
