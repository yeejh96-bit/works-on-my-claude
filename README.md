...
- **`verify` 서브에이전트** — 테스트·실행 검증을 별도 컨텍스트에서 끝내고 **통과/실패만** 가져온다. 긴 실행 로그가 본 대화에 쌓이지 않는다.
...
- **settings.json** — 비밀키 파일(`.env`) 읽기를 차단해 입문자를 보호하고, Windows(PowerShell)에서 자주 쓰는 읽기 전용 명령을 미리 허용한다. (Bash의 `git status` 등 읽기 전용 명령은 Claude Code가 기본으로 프롬프트 없이 허용한다.)
