...
>
> **2026-09-01 현재 열려 있는 것은 3건이다** — 아래 셋뿐이다. 셋 다 닫는 조건은 분명하고, `open:import-command` 만 계기(기존 프로젝트에 `/womc` 를 까는 일)를 기다린다.
...
- [ ] **입력 리다이렉션(`cat < .env` 류)이 골격의 `.env` deny 를 우회하는지 실측 — 급하지 않음 · 다음에 `.env` 를 쓰는 폴더에서 겸사** <!-- open:env-deny-redirect -->
  ...
  - 확인 방법: 아무 프로젝트에 더미 `.env`(가짜 값만 넣는다)를 두고 Bash 로 `cat < .env` 를 시켜 차단되는지 본다.
    `python3 scripts/check-sync.py` 로는 못 잡는다(설정이 파일에 있는지만 본다).
...
- [ ] **`/import` 가 womc 온보딩 병합과 겹치거나 충돌하는지 실측 — 급하지 않음 · 다음에 기존 프로젝트에 `/womc` 를 깔 때 겸사** <!-- open:import-command -->
  ...
  - 확인 방법: 기존 코드가 있는 아무 폴더에서 `/import` 를 돌려 무엇을 끌어오는지 보고,
    ...
    `python3 scripts/check-sync.py` 로는 못 잡는다(문구가 파일에 있는지만 본다).
  - **닫히는 조건 있음, 트리거 없음**: 기존 프로젝트에 `/womc` 를 까는 일이 생겨야 확인된다.
    그 일이 언제 생길지는 정해져 있지 않다.

- [ ] **프로젝트 스킬(`.claude/skills/`)이 서브에이전트 스킬 목록에 실제로 뜨는지 실측 — 급하지 않음 · 다음에 스킬이 있는 폴더에서 겸사** <!-- open:project-skill-to-subagent -->
  - 2026-09-01 v3.9.0 이 연 항목이다. 그 작업에서 실측으로 확인한 것은 **플러그인 스킬(`womc:*`)과 내장 스킬이 서브에이전트에 주입된다**는 것까지다.
    **프로젝트 스킬(`.claude/skills/<이름>/SKILL.md`)로는 못 봤다** — 이 저장소엔 `.claude/skills/` 폴더가 없고,
    세션 도중 새로 만든 파일은 그 세션에서 안 잡힌다(임시 에이전트 파일로 확인: `Agent type not found`).
  - 왜 중요한가: v3.9.0 의 목적 자체가 **각 프로젝트에서 앞으로 만들 스킬**을 서브에이전트가 쓰게 하는 것이다.
    플러그인 스킬만 되고 프로젝트 스킬이 안 되면 목적이 반쯤 빈다. 같은 장치가 만드는 같은 목록이라 될 가능성이 높지만, **확실한 것과 확인한 것은 다르다.**
  - 손댈 파일: 없음(관찰만 한다). 안 뜨는 것으로 판명되면 `CLAUDE.md`·`commands/womc.md` 의 경계 문장에서
    「`.claude/skills/` 의 프로젝트 스킬까지다」를 사실에 맞게 고치고, 위임 프롬프트에 `SKILL.md` 경로를 적어 Read 하게 하는 우회책을 `CLAUDE.md` 「적극 위임」에 한 줄 더한다.
  - 이어 쓸 것: 경계 문장의 정본은 `CLAUDE.md` 「프로젝트 스킬·규칙」 세 번째 불릿이다
    (같은 문장이 `commands/womc.md` 에도 있다 — 한쪽만 고치면 `scripts/check-sync.py` 1번이 DRIFT 로 잡는다).
  - 끝난 것으로 보는 조건: 프로젝트 스킬이 하나라도 있는 폴더에서 서브에이전트가 그 스킬 이름을 자기 목록에서 읽어 냈는지 사람이 화면으로 확인하고,
    안 뜨면 위 경계 문장까지 고친 상태.
  - 확인 방법: `.claude/skills/` 에 스킬이 하나라도 있는 폴더에서 **새 세션**을 열고 `womc:explore` 에
    「네 시스템 프롬프트의 스킬 목록에 있는 이름을 전부 나열해라」라고 시킨다. 그 프로젝트 스킬 이름이 목록에 있으면 통과.
    `python3 scripts/check-sync.py` 로는 못 잡는다.
  - **닫히는 조건 있음(지금도 가능)**: `make-rule` 로 스킬을 하나 만든 뒤 새 세션에서 물어보면 1분이면 끝난다.
