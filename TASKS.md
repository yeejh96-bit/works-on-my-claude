...
- [ ] **v2.4.0 확인 — 다음 기능 작업 한 번으로 겸사 본다 (2026-08-12 남김)**
  ...
  - 확인 방법: 다음 기능 작업 때 화면을 본다. **`scripts/check-sync.py` 는 이걸 못 잡는다**(문구 존재만 확인해 준다).
...
- [x] 설계 단계에 「안 고른 길」 + 되돌리기 어려운 일에 승인 관문 (v2.4.0) — **2026-08-12**
  - 근거: 사용자가 가져온 '그래프 엔지니어링' 글(그렉 아이젠버그)과 womc 를 대조했다. 글이 말하는 잡·화살표·상태,
    ...
    ③ **승인 관문 목록이 없다** — `push` 만 명시돼 있고 파일 삭제·이동·배포는 문구가 없었다(`commands/womc.md:239` 의 settings.json 주석 한 줄이 전부였다).
  ...
  - 확인 방법: `PYTHONIOENCODING=utf-8 py scripts/check-sync.py` → 전 항목 OK, 버전 `2.4.0`.
    **`agents/plan.md` 와 `skills/plan-feature/SKILL.md` 는 `commands/womc.md` 에 임베드되지 않아 스크립트 대조 대상이 아니다** —
