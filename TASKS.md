...
- ✅ **2026-08-11 — `/code-review` 가 이 파일에서 잡은 5건을 고쳤다 (버전 변경 없음, 기록 파일만).**
  ...
  - ② 「다음 `/womc update` 때 겸사」를 실제로 알려 줄 장치가 없던 문제 — 두 건을 `docs/HARNESS-AUDIT.md`
    (인용문 + 4번 + 5번③)에도 「열려 있음」으로 적었다. 자동 감사가 그 파일부터 읽으므로 다음 갱신 때 눈에 걸린다.
  ...
  - ⑤ 「할 일」 맨 위가 보류 항목이던 문제 — **열린 것 / 닫힌 것·보류** 두 소절로 재배치하고,
    보류 항목(`force-for-plugin`)은 `[x]` 로 닫아 집히지 않게 했다(완료가 아니라 선반에 올린 것 — 그 뜻을 항목에 적었다).
  - **다음에 이 두 파일 중 하나를 고칠 때는 다른 하나도 같이 본다** — ②의 장치가 두 파일이 맞물려 있어야 작동한다.

- ✅ **하네스 감사가 남긴 실측 3건 — 전부 끝났다 (2026-08-10).**
  다음에 할 일은 **새 기능**이다 — 「할 일」에 열려 있는 2건은 전부 「급하지 않음」이라 붙잡을 필요가 없다.
...
> **각 항목의 배경·근거 URL 이 어디 있는지는 항목마다 따로 적었다** — 두 건이 서로 다른 절에 있어 한 곳으로 뭉쳐 가리킬 수 없다.
> (`subagentStatusLine` 은 `docs/HARNESS-AUDIT.md` v2.1.1 기록 **4번 절**의 「새로 챙길 만한 것」이고,
> v2.2.0 갱신 모드 확인은 v2.1.1 기록 **5번③ 의 「이후 이력」**에 적어 두었다 — 그 확인 자체는 v2.2.0 작업이 새로 만든 것이다.)
> **2026-08-11 — 남은 두 항목은 「급하지 않음」으로 내렸다.** 둘 다 확인일 뿐이고 실패해도 세팅이 깨지지 않는다.
...
> 안 된다. 그래서 **열린 항목을 위로, 보류·닫힌 항목을 아래로** 재배치했다. 위쪽 `[ ]` 두 건은 둘 다 「급하지 않음」이라
> 지금 집을 일이 아니고, 새 기능을 시작하는 것이 맞다. 맨 아래 「보류」 항목은 **집지 않는다.**
>
> **잊히지 않게 하는 장치**: 이 두 건은 `docs/HARNESS-AUDIT.md` v2.1.1 기록의 **인용문·4번·5번 절에 「열려 있음」으로도
> 적어 두었다.** `/womc update` 7번 단계가 자동 실행하는 `harness-audit` 는 그 파일부터 읽으므로(`skills/harness-audit/SKILL.md` 1단계),
> 다음 갱신 때 이 두 건이 저절로 눈에 걸린다. **한쪽만 고치면 이 장치가 끊긴다** — 두 파일을 항상 같이 고친다.

### 열려 있는 것 (둘 다 급하지 않음)

- [ ] **v2.2.0 갱신 모드가 옛 `allow` 4줄을 실제로 지우는지 확인 — 급하지 않음 · 다음 `/womc update` 때 겸사 확인 (2026-08-11 결정)**
  ...
  - 굳이 전용 시험 폴더를 만들 거라면 `.claude/settings.json` 에 4줄 + 사용자 항목 1줄(예: `Bash(npm test:*)`)을 넣어 두고 돌린다.
    단 플러그인 재설치 + **Claude Code 재시작** 뒤라야 의미가 있다 — 이 저장소 안에서는 못 한다.

- [ ] **`subagentStatusLine`·`/statusline` 을 골격에 들일지 판단 — 급하지 않음 · 생각날 때 (감사 v2.1.1 기록 4번 「새로 챙길 것」)**
...
- [x] ~~**`force-for-plugin: true` 가 이 판에서 실제로 먹는지 확인 (감사 5번②)**~~ — **보류로 닫았다. 집지 않는다.**
  A안으로 고쳤으므로 지금은 필요 없다. 나중에 "프로젝트마다 `outputStyle` 을 박지 않게" 하고 싶어질 때 **새 항목으로 다시 연다.**
  (열린 항목으로 두면 「할 일」을 위에서부터 집는 세션이 보류 항목을 붙잡게 되므로 `[x]` 로 닫았다 — 완료가 아니라 **선반에 올린 것**이다.)
