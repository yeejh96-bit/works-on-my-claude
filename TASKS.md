...
- [ ] **v2.2.3 의 실사용 검증 — 한 줄짜리 「설명 방식」 절이 실제로 최신으로 교체되는지 사람이 본다 (2026-08-11 남김)**
  - **다음 세션은 이 항목부터 집는다.** 구현은 v2.2.3 으로 끝났다(아래 「끝난 일」). **남은 것은 돌려 보는 것 하나뿐이다.**
  - **⚠ 먼저 push + `claude plugin update womc@works-on-my-claude` + Claude Code 재시작.** 안 하면 옛 캐시(`2.2.2`)로 돌아 증거가 안 된다.
  - **시험대는 `pay` 다** (`C:\Users\s2\Documents\project\pay`). 그 폴더 「설명 방식」 절이 아직 `- 모든 설명은 한국어로 한다.` 한 줄로 남아 있다
    — v2.2.2 가 오판해 보존해 버린 그 절이 그대로 있어 시험 재료가 된다. **`pay` 는 git 저장소라 되돌릴 수 있다**(백업도 `<scratchpad>/pay-backup/`).
  - 끝난 것으로 보는 조건: `/womc update` 뒤 그 절이 **최신 두 줄**(`- 모든 설명·보고는 한국어로 한다.` + 출력 스타일 안내)로 바뀐다.
    완료 보고에 「교체했다」가 뜬다. 앞서 넣어 둔 `## 시험용 내 규칙` 절은 **여전히 그대로 남아 있어야 한다**(v2.2.2 회귀 확인).
  - 확인 방법: 갱신 전후로 `pay/CLAUDE.md` 를 눈으로 비교한다. **`scripts/check-sync.py` 는 이걸 못 잡는다.**
  - **v2.3.0 도 같은 자리에서 함께 본다 (2026-08-12 추가)**: 그 `/womc update` 때 Claude Code 가 패치만 오른 상태(앞 두 자리가 같음)면
    **감사 본체(신기능 조사)가 안 도는지**, 그러면서도 열린 확인 목록 알림과 "그래도 지금 조사할까요?" 한 줄은 뜨는지 함께 확인한다.
...
- [x] 「설명 방식」 절 판정이 아주 옛 골격 판을 못 잡던 빈틈을 메움 (v2.2.3) — **2026-08-11**
  ...
  - 확인 방법: `PYTHONIOENCODING=utf-8 py scripts/check-sync.py` → 9항목 전부 OK, 버전 `2.2.3`.
    **실제로 교체되는지는 스크립트로 못 잡는다** — 위 「지금 하는 일」의 v2.2.3 실사용 검증에서 사람이 확인한다.
...
> **지금 집을 것은 이 소절이 아니라 「지금 하는 일」 맨 위의 v2.2.3 실사용 검증이다.**
...
- [x] ~~**④ 「설명 방식」 절 판정이 아주 옛 골격 판을 「사용자 커스텀」으로 오판한다**~~
  ...
  **실제로 교체되는지는 아직 사람이 못 봤다** — 「지금 하는 일」의 v2.2.3 실사용 검증이 그것이다.
