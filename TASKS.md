...
- ✅ **하네스 감사가 남긴 실측 3건 — 전부 끝났다 (2026-08-10).**
  ...
  - **2) `permissions.allow` 의 PowerShell 4줄이 필요한지 (감사 5번③)** — **필요 없다. 뺄 수 있다.**
    ...
    - **골격에서 실제로 4줄을 빼는 일은 이 항목이 하지 않는다** — 「할 일」로 넘겼고, **v2.2.0 으로 끝났다**
      (`docs/CHANGELOG.md` 의 v2.2.0 항목 · 「할 일」의 「닫힌 것 · 보류」 소절에 `[x]` 로 있다).
...
> 근거 URL 은 `docs/CHANGELOG.md` 의 「열린 확인 4건 닫음」 항목에 있다. **지금 열려 있는 확인은 4건이다.**
...
- [ ] **v2.13.0 확인 — `PLAN.md` 가 끝난 뒤 줄어드는지 화면으로 본다 (2026-08-28 남김)**
  ...
  - 손댈 파일: 없음(관찰만 한다). 이어 쓸 것: `skills/plan-feature/SKILL.md` 0절 하위절 「이미 부풀어 있는 PLAN.md 를 만나면」 ·
    4절 하위절 「단계가 끝나면 — PLAN.md 에서도 경위를 뺀다」 · `.claude/rules/제약-공통.md` 의 절 이름 「사용자가 닫은 것」·「나중에 · 안 할 것」.
  - 끝난 것으로 보는 조건 — 셋 다 봐야 한다:
...
- [ ] **v2.12.0 확인 — 제약이 `.claude/rules/` 로 가는지 화면으로 본다 (2026-08-28 남김)**
  ...
  - 손댈 파일: 없음(관찰만 한다). 이어 쓸 것: `skills/plan-feature/SKILL.md` 0절 하위절 「이미 제약이 쌓인 TASKS.md 를 만나면」 ·
    4절 하위절 「위임하기 전에 — 그 단계에 걸린 제약을 프롬프트에 넣는다」 · 파일명 규약 `.claude/rules/제약-<영역>.md`.
  - 끝난 것으로 보는 조건 — 셋 다 봐야 한다:
