...
- [~] **갱신 모드의 `CLAUDE.md` 처리 버그 2건 수정 — 설계까지 끝냈고 구현은 안 했다 (2026-08-11 중단, 사용자가 이어서 하기로 함)**
  - **다음 세션은 이 항목부터 집는다.** 고칠 대상 2건의 배경·증상 전문은 아래 「할 일」 → 「그 밖의 할 일」의 **①②** 에 있다. 먼저 그것부터 읽는다.
  - **손댈 파일은 `commands/womc.md` 하나다** — 갱신 모드 1번의 `CLAUDE.md` 항목(ⓐ 분기 + 「설명 방식」 절 보존 지시).
    작업 크기는 「작은 변경」이지만 **버전을 올리는 변경이라** `PLAN.md` 버전 이력과 이 파일 「끝난 일」을 함께 갱신해야 한다(다음 버전은 `2.2.2`).
  - **설계 초안 (이대로 구현하면 된다 — 다시 설계하지 말 것):**
    1. ⓐ 분기를 「통째로 덮어쓴다」에서 **「절 단위로 병합한다」** 로 바꾼다. 기존 파일을 `## ` 제목 기준으로 절로 쪼갠다.
    2. **골격 절 제목 8개**(`## 나에 대해` · `## 코드 제공 방식` · `## 설명 방식` · `## 서브에이전트 보고 규약` ·
       `## 적극 위임 (오케스트레이터 방식)` · `## 세션을 이어서 하기` · `## 절차 지키기 (벗어나면 되돌리기)` · `## 프로젝트 상세`)에
       해당하는 절만 최신 골격 내용으로 교체한다.
    3. **그 목록에 없는 제목의 절은 사용자 것이다 — 원래 순서 자리에 그대로 남긴다**(바로 앞 골격 절 뒤에 붙여 둔다).
       골격에는 있는데 기존 파일에 없는 절은 골격 순서대로 새로 넣는다.
    4. 「설명 방식」 절만 예외 처리: **폐기된 표식 `womc:brevity=` 나 `answer-style.js` 가 그 절 안에 보이면 옛 골격 판이므로 최신으로 교체**한다.
       그 표식이 없고 골격 최신 문구와도 다르면 사용자가 고친 것으로 보고 지금처럼 보존한다.
    5. 완료 보고에 **「남긴 사용자 절 제목 목록」과 「설명 방식 절을 교체했는지 보존했는지」** 를 한 줄씩 적는다.
  - 이어 쓸 것: 위 골격 절 제목 8개 목록. `commands/womc.md` 1번 절의 `CLAUDE.md` 템플릿 본문에서 그대로 뽑은 것이다 —
    **골격 절 제목을 바꾸면 이 목록도 같이 고쳐야 한다**(안 고치면 그 절이 「사용자 것」으로 오인돼 옛 판이 남는다).
  - 끝난 것으로 보는 조건: 아래 ①② 두 항목의 「끝난 것으로 보는 조건」이 둘 다 충족되고, 버전이 `2.2.2` 로 올라가며
    `PLAN.md`·이 파일의 기록이 함께 갱신된다.
  - 확인 방법: `py scripts/bump-version.py 2.2.2` 로 버전을 올린 뒤 `PYTHONIOENCODING=utf-8 py scripts/check-sync.py` → 전 항목 OK.
    **동작 자체는 스크립트가 못 잡는다** — 골격 `CLAUDE.md` 중간에 `## 시험용 내 규칙` 절을 끼운 시험 폴더에서
    `/womc update` 를 돌려 그 절이 남는지 사람이 본다.
    시험대 후보: `C:\Users\s2\Documents\project\easy-welfare` · `...\pay` · `...\easy-welfare\reference\pay` (셋 다 아직 옛 골격이다).
  - ⚠ **시험대 하나는 이미 썼다** — `ax` 는 오늘 `2.2.1` 로 갱신돼 더는 옛 골격 시험대가 아니다. 백업은 `<scratchpad>/ax-backup/` 에 있다.
...
> **2026-08-11 현재 그쪽에 갱신 모드 버그 3건이 쌓여 있고, 그중 ①은 사용자 글을 지우는 심각한 것이다** — 확인 2건보다 이쪽이 급하다.
...
- [ ] **① 갱신 모드가 사용자가 `CLAUDE.md` 에 덧붙인 절을 통째로 날린다 — 가장 급하다 (2026-08-11 실측)**
  - **어떻게 드러났나**: `C:\Users\s2\Documents\project\ax` 에 `/womc update` 를 돌리다 발견했다. 그 파일 첫 줄이 골격 그대로라
    갱신 모드 1번의 **ⓐ 분기(「통째로 덮어쓴다」)** 에 걸리는데, 파일 중간(「절차 지키기」와 「프로젝트 상세」 사이)에
    사용자가 쓴 **「절영봇 절대 원칙」 7개**(개인정보를 LLM 에 보내지 말 것 등)가 들어 있었다. 그대로 덮었으면 **전부 소실**이다.
  - **왜 생기나**: ⓐ 가 보존하는 것은 **「## 설명 방식」 절 하나뿐**이다. 사용자가 골격 사이사이에 새 절을 끼워 넣는 것은
    아주 흔한 사용법인데(온보딩 병합 구획 `womc:begin/end` 를 쓰지 않은 경우), 그 경우가 전혀 고려돼 있지 않다.
  - **이번엔 어떻게 넘겼나**: 사람이 알아채고 손으로 병합했다 — 최신 골격 본문에 그 절만 「프로젝트 상세」 앞으로 되살렸다.
    자동으로 그렇게 되도록 고치는 것이 이 항목이다.
  - 손댈 파일: `commands/womc.md` 갱신 모드 **1번의 `CLAUDE.md` ⓐ 분기**.
  - 이어 쓸 것: 골격 절 제목 목록(`## 나에 대해` · `## 코드 제공 방식` · `## 설명 방식` · `## 서브에이전트 보고 규약` ·
    `## 적극 위임 (오케스트레이터 방식)` · `## 세션을 이어서 하기` · `## 절차 지키기 (벗어나면 되돌리기)` · `## 프로젝트 상세`).
    **골격에 없는 제목의 절은 사용자 것**이라는 판정이 이 목록에서 나온다.
  - 끝난 것으로 보는 조건: ⓐ 분기가 **절 단위로 판정**한다 — 골격 제목의 절만 최신으로 교체하고,
    **골격에 없는 제목의 절은 원래 순서 자리에 그대로 남긴다.** 무엇을 남겼는지 완료 보고에 한 줄 적는다.
  - 확인 방법: 골격 `CLAUDE.md` 중간에 `## 시험용 내 규칙` 절을 끼운 시험 폴더에서 `/womc update` 를 돌려
    그 절이 남아 있는지 본다. (`scripts/check-sync.py` 는 이걸 못 잡는다 — 사람이 본다.)

- [ ] **② 「설명 방식」 절 보존 규칙이 그 절을 옛 골격에 영구히 묶는다 (2026-08-11 실측)**
  - **어떻게 드러났나**: 같은 `ax` 폴더에서, `CLAUDE.md` 머리 표식은 `1.20.0` 인데 **「## 설명 방식」 절 본문만 `1.14.0`/`1.15.0` 판**이었다.
    이미 지워진 `.claude/answer-style.js` 훅을 가리키는 문장과 폐기된 `womc:brevity=` 강도 체계가 그대로 남아 있었다.
  - **왜 생기나**: 갱신 모드 1번은 그 절을 **내용을 보지 않고 무조건 되돌려 놓는다.** 사용자가 고친 절이든 옛 골격 원문이든 똑같이 취급하므로,
    한 번 갱신을 거친 프로젝트는 그 절만 영영 안 올라간다.
  - **이번엔 어떻게 넘겼나**: 그 절이 옛 캐시(`~/.claude/plugins/cache/works-on-my-claude/womc/1.14.0` 등)의 골격 원문과
    **공백 무시 일치**하는 것을 확인해 사용자 수정이 아니라고 판정하고, 최신 골격 것으로 교체했다.
  - 손댈 파일: `commands/womc.md` 갱신 모드 **1번의 「설명 방식」 절 보존 지시**.
  - 이어 쓸 것: 위 판정 방법 — 옛 골격 원문과 같으면 사용자 수정이 아니다. 옛 판본은 로컬 플러그인 캐시 폴더들에 남아 있다.
  - 끝난 것으로 보는 조건: 보존 지시에 **「그 절이 옛 골격 판본과 같으면 사용자 수정이 아니므로 최신 것으로 교체한다」** 가 들어간다.
    (캐시 대조가 어려우면, 최소한 그 절 안에 폐기된 표식 `womc:brevity=` 나 `answer-style.js` 가 보이면 옛 골격으로 보고 교체하게 한다.)
  - 확인 방법: 옛 골격이 깔린 폴더에서 `/womc update` 를 돌린 뒤 「설명 방식」 절이 최신 두 줄인지 본다.
