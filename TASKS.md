...
- [ ] **v2.2.0 갱신 모드가 옛 `allow` 4줄을 실제로 지우는지 확인 — 급하지 않음 · 다음 `/womc update` 때 겸사 확인 (2026-08-11 결정)**
  - **일부러 전용 시험 폴더를 만들지 않는다.** 다음에 아무 프로젝트에서 `/womc update` 를 쓸 일이 생겼을 때,
    그 폴더의 `.claude/settings.json` 의 `allow` 를 한 번 훑어보는 것으로 대신한다.
  - **급하지 않은 이유**: 이 확인이 실패해도 세팅이 깨지지 않는다. 옛 4줄이 남을 뿐이고, 그 4줄은 읽기 전용 명령 허용이라 해롭지 않다.
    (이 저장소 자신의 `.claude/settings.json` 은 이미 `allow: []` 로 v2.2.0 이 반영돼 있다.)
...
- [ ] **`subagentStatusLine`·`/statusline` 을 골격에 들일지 판단 — 급하지 않음 · 생각날 때 (감사 4번 「새로 챙길 것」)**
