...
2. 항상 로드되는 규칙(CLAUDE.md) + 프로젝트 명세(SPEC.md, `@import`)만 always-on으로 유지. 출력을 짧게 하는 강도 4단계(`womc:brevity` — `케이브맨`/`약하게`/`보통`/`최소`, 기본 `케이브맨`)가 CLAUDE.md 「설명 방식」 절에 표식으로 들어 있으며, 대화 답변에만 적용된다(파일 문서는 대상 아님).
