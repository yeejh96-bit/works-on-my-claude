...
## 3. 핵심 기능 (3~5개)
1. `/womc` — 고정 골격 생성(이미 있는 파일은 덮어쓰지 않고 건너뜀). 신규 빈 폴더면 빈 SPEC 템플릿, **기존 코드가 있는 폴더면 `explore` 로 훑어 SPEC 초안까지 채움**(사용자 검토 전제). `/womc update` 는 불변 골격만 최신으로 교체(사용자가 채운 파일은 보존).
...
- `/womc` 가 코드를 구현하지 않음(구조 생성까지만). SPEC 도 대신 채우지 않음 — 단, **기존 프로젝트 감지 시 코드를 훑어 SPEC 초안 작성은 예외**(사용자 검토 전제).
- GUI/웹 UI 없음.
