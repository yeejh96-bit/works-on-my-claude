...
4. 운영 스킬 2종: `plan-feature`(기능 추가·수정 겸용 — 큰 작업은 쪼개 PLAN/TASKS 관리, 작은 변경·버그수정·리팩터는 가벼운 경로, 조사→설계→구현→검증 위임 후 검토는 `/code-review` 권유. 세션이 바뀌어도 이어지도록 각 작업 항목에 손댈 파일·이어 쓸 것·완료 조건·확인 방법을 남긴다) · `make-rule`(규칙 자동 생성).
