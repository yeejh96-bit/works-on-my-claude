...
3. `.claude/settings.json` 은 정본에 있는 키만 견준다. 정본에 없는 키(`hooks`·`autoMemoryDirectory`·`enabledPlugins` 등)와 사용자가 더한 allow·deny 는 그대로 둔다. 값이 정본과 다르면 커밋 기록을 보고 일부러 바꾼 것이면 남긴다. 상태줄 파일은 정본으로 맞춘다.
4. 브리핑이 규격(칸 넷 · 칸 밖 줄 · 굵게·이모지·꾸밈용 표 · 40줄)에 어긋나면 고치지 말고 한 줄로 알리기만 한다. 정리는 평소 작업 시작 때 「부푼 것」 규칙이 맡는다.
5. 브리핑이 그대로인지 기계로 본다 — `diff <(git show HEAD:CLAUDE.md | sed -n '/^### /,$p') <(sed -n '/^### /,$p' CLAUDE.md)` 가 빈 결과여야 한다.
6. 커밋하고 「바꾼 것」을 한 줄 보고한다. 묻지 않는다 — 다만 프로젝트 규칙이 하네스 파일을 고치기 전에 묻기를 요구하면 그쪽을 따른다.
