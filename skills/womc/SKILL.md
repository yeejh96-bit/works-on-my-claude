...
옛 하네스 지우기는 하지 않는다. 브리핑 내용은 규격에 어긋나는 자리만 손댄다.
1. 체크포인트 커밋. womc 가 만들었던 옛 파일(`agents/`·plan-feature 스킬·`PLAN.md`·`docs/CHANGELOG.md`)이 아직 남아 있으면 「B. 다시 짜기」로 간다.
2. `CLAUDE.md` 의 첫 줄부터 브리핑 설명문 끝까지를 정본으로 바꾼다. `### 할 일` 아래는 그대로 둔다. `.claude/settings.json`·상태줄도 정본에 맞춘다(사용자가 더한 allow·deny 는 남긴다).
3. 브리핑이 규격에 맞는지 본다 — 칸 넷 · 칸 밖에 적힌 줄 · 굵게·이모지·꾸밈용 표 · 40줄. 어긋난 자리만 고친다.
4. 칸을 옮기거나 이름을 바꿨으면 B 6 과 같이 줄 단위 대조와 참조 수선을 한다.
5. 커밋하고 「바꾼 것」을 한 줄 보고한다.
