...
## 0. 먼저 확인
- 현재 폴더에 `CLAUDE.md` 가 있고 그 안에 `womc:skeleton-version=` 표식이 있으면 → 「B. 다시 짜기」. 표식 버전이 이 스킬 맨 위 표식과 같으면 브리핑만 본다 — 40줄을 넘거나 칸이 넷 밖이거나 칸 밖에 적힌 줄이 있으면 브리핑 기준대로 정리하고, 아니면 할 일이 없다고 한 줄 알리고 끝낸다.
...
3. 남길 것은 **코드로 알 수 없는 사실**뿐이다.
   ...
   - 브리핑은 칸 넷·40줄 안에 들어와야 한다. 넘치면 주제별로 `docs/` 로 가른다.
...
- 앞으로 계속 지킬 제약은 .claude/rules/<이름>.md 에 남기되 반드시 paths 를 붙인다(그 파일을 만질 때만 읽히게). 무엇을 적을지는 브리핑과 같은 기준이다. 전체에 걸치는 규칙은 브리핑에 한 줄로.
...
- 작업 시작 때 부푼 것 — 끝난 항목이 남은 TASKS.md, paths 없는 규칙, 200줄 넘는 이 파일, 40줄 넘는 브리핑, 넷 밖의 브리핑 칸, 브리핑 기준에 안 맞는 브리핑·.claude/rules 줄, 브리핑에 쌓인 배경·근거·긴 목록 — 이 보이면 묻지 않고 정리하고 한 줄 보고한다.

## 브리핑 (모델이 채우고 고친다)
코드로 알 수 없는 것만 적는다. 칸은 「할 일 · 지킬 것 · 끝난 조건 · 확인 방법」 넷뿐이다 — 필요한 것만 두고 새 칸을 만들거나 이름을 바꾸지 않는다. 모든 줄은 칸 안에 적는다(이 설명문과 첫 칸 사이에 아무것도 적지 않는다). 「할 일」은 프로젝트가 무엇인지 한 문단이지 진행 기록이 아니다. 어느 칸에도 어디까지 했나·다음에 뭘 하나·끝난 일은 적지 않는다(TASKS.md 와 커밋이 맡는다).
...
브리핑에는 지시문 한 줄씩만 둔다. 그 지시의 배경·근거·숫자·날짜·결정 경위·긴 목록은 처음부터 docs/<주제>.md 에 쓰고, 브리핑에는 「…할 때는 docs/<주제>.md 를 읽는다」 한 줄만 남긴다. 길어져서 옮기는 게 아니라 애초에 나눠 쓴다 — 이 파일은 매 작업 통째로 읽히고 docs/ 는 그 일을 할 때만 읽힌다. 브리핑은 첫 칸부터 파일 끝까지 40줄을 넘기지 않는다. 넘으면 넘는 만큼 docs/ 로 가른다. 표·굵게·이모지는 쓰지 않는다.
