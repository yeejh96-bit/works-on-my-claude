...
1. 요청이 모호하면 완전히 이해될 때까지 선택지로 되묻는다. 답이 달라도 결과가 크게 안 달라지면 묻지 않는다. 끝나면 「무엇을 만들고 · 무엇은 안 만들고 · 어떻게 확인할지」를 쉬운 말로 다섯 줄 안에 보여 주고 시작한다. 앞으로도 지킬 것이 있으면 브리핑 기준대로만 적는다. 이번 요청에서 「만들 것·안 만들 것」은 채팅에만 보이고 브리핑에는 옮기지 않는다.
...
- 일하다 틀린 가정·안 되는 도구·이 프로젝트에서만 다르게 해야 하는 것을 알게 되면 사용자가 말하지 않아도 브리핑이나 .claude/rules 에 남기고 한 줄 알린다.
- 작업 시작 때 부푼 것(끝난 항목이 남은 TASKS.md · paths 없는 규칙 · 200줄 넘는 이 파일 · 브리핑 기준에 안 맞는 줄(브리핑·.claude/rules))이 보이면 묻지 않고 정리하고 한 줄 보고한다.

## 브리핑 (모델이 채우고 고친다)
코드로 알 수 없는 것만 적는다. 보통 「할 일 · 지킬 것 · 끝난 조건 · 확인 방법」이 들어가지만 필요한 것만 적고 없는 칸은 만들지 않는다. 「할 일」은 프로젝트가 무엇인지 한 문단이지 진행 기록이 아니다. 어디까지 했나·다음에 뭘 하나·끝난 일은 적지 않는다(TASKS.md 와 커밋이 맡는다).
확인 방법은 「명령 한 줄 + 기대 출력 한 줄」로 적는다. 사람 눈으로만 아는 것은 명령 대신 화면으로 확인한다.
사용자가 한 말 중 앞으로의 작업 결과를 바꾸는 것만 「…한다/…하지 않는다」 지시문으로 여기로 옮긴다. 적기 전에 「이 줄이 없으면 다음 작업이 달라지나」를 묻고, 아니면 적지 않는다. 의향·계획·사정(예: 유료화할 생각이 있다)은 그대로 적지 말고, 그것이 요구하는 지시로 바꿀 수 있을 때만 그 지시를 적는다. 브리핑이 바뀌면 .claude/rules 와 .claude/settings.json 도 그에 맞춘다.
