...
- 작업 시작 때 부푼 것 — 끝난 항목이 남은 TASKS.md, paths 없는 규칙, 200줄 넘는 이 파일, 브리핑 기준에 안 맞는 브리핑·.claude/rules 줄, 브리핑에 쌓인 배경·근거·긴 목록 — 이 보이면 묻지 않고 정리하고 한 줄 보고한다.

## 브리핑 (모델이 채우고 고친다)
코드로 알 수 없는 것만 적는다. 보통 「할 일 · 지킬 것 · 끝난 조건 · 확인 방법」이 들어가지만 필요한 것만 적고 없는 칸은 만들지 않는다. 「할 일」은 프로젝트가 무엇인지 한 문단이지 진행 기록이 아니다. 어느 칸에도 어디까지 했나·다음에 뭘 하나·끝난 일은 적지 않는다(TASKS.md 와 커밋이 맡는다).
...
브리핑에는 지시문 한 줄씩만 둔다. 그 지시의 배경·근거·숫자·날짜·결정 경위·긴 목록은 처음부터 docs/<주제>.md 에 쓰고, 브리핑에는 「…할 때는 docs/<주제>.md 를 읽는다」 한 줄만 남긴다. 길어져서 옮기는 게 아니라 애초에 나눠 쓴다 — 이 파일은 매 작업 통째로 읽히고 docs/ 는 그 일을 할 때만 읽힌다.
