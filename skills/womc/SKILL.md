...
## 기능 작업 순서
1. 새 기능 요청이 모호하면 완전히 이해될 때까지 선택지로 되묻는다. 답이 달라도 결과가 크게 안 달라지면 묻지 않는다. 끝나면 「무엇을 만들고 · 무엇은 안 만들고 · 어떻게 확인할지」를 쉬운 말로 다섯 줄 안에 보여 주고 시작한다. 앞으로도 지킬 것은 브리핑에 적는다.
...
3. 체크포인트 커밋.
4. 구현. 무거운 조사·구현·확인은 서브에이전트에 맡기고 결론만 받는다.
5. 끝났다고 말하기 전에 브리핑의 확인 방법을 별도 서브에이전트가 실행한다. 만든 쪽이 스스로 판정하지 않는다.
