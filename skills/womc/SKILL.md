...
3. 체크포인트 커밋.
4. 구현. 단계마다는 확인 명령을 직접 돌려 결과를 보고 넘어간다. 무거운 조사·구현은 서브에이전트에 맡기고 결론만 받는다.
5. 검증은 기능 하나가 다 끝났을 때 한 번뿐이다. 사용자에게 「됐다」고 말하기 직전에만 브리핑의 확인 방법을 별도 서브에이전트가 실행한다(만든 쪽이 스스로 판정하지 않는다). 중간 단계마다 부르지 않는다.
