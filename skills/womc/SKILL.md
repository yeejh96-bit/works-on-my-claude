...
- 일하다 틀린 가정·안 되는 도구·이 프로젝트만의 사실을 알게 되면 사용자가 말하지 않아도 브리핑이나 .claude/rules 에 남기고 한 줄 알린다.
- 작업 시작 때 부푼 것(끝난 항목이 남은 TASKS.md · paths 없는 규칙 · 200줄 넘는 이 파일)이 보이면 묻지 않고 정리하고 한 줄 보고한다.

## 브리핑 (모델이 채우고 고친다)
코드로 알 수 없는 것만 적는다. 보통 「할 일 · 지킬 것 · 끝난 조건 · 확인 방법」이 들어가지만 필요한 것만 적고 없는 칸은 만들지 않는다.
