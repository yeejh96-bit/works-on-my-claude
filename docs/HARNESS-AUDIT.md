...
> **단 ③에서 파생된 열린 확인이 있다** — 아래 「열린 확인」 목록을 본다.
...
> - `open:allow-cleanup` — v2.2.0 갱신 모드가 옛 `allow` 4줄을 지우는지 (파생 자리: 아래 5번③ 「이후 이력 ②」)
...
### 5. 실측 필요 — **5건은 전부 닫혔다 · 단 ③에서 파생된 열린 확인 1건이 있다 (`open:allow-cleanup`).**
①~④ 는 v2.1.0 기록에서 그대로 이월된 것이고, ⑤ 가 이번에 새로 생겼다.
닫힌 것: ①(확인됨) · ②(2026-08-11 「보류」) · ③(확인됨 — 뺄 수 있음 → v2.2.0 으로 실행) · ④(불필요) · ⑤(확인됨 — 유지).
**파생된 열린 확인**은 ③의 「이후 이력 ②」에 있다 — 갱신 모드 청소가 실제로 도는지(정본: `TASKS.md` 「할 일」).
...
③ ~~**`permissions.allow` 의 PowerShell 4줄이 필요한지**~~ →
   ...
   **판정의 범위**: "실사용에서 프롬프트가 안 뜬다"까지가 확인된 것이다. 그것이 내장 자동 허용 때문인지
   ...
   >
   > **이후 이력 ② — 여기서 파생된 열린 확인 1건 (2026-08-11 현재 「급하지 않음」).**
   > **그 청소 분기가 실제로 도는지는 아직 확인하지 못했다** — `check-sync.py` 로는 잡히지 않는다(스크립트는 문구 대조만 한다).
   > → 열린 확인 `open:allow-cleanup` (정본: `TASKS.md` 「할 일」 — 성립 조건·경고·확인 방법은 거기 한 곳에만 있다).
