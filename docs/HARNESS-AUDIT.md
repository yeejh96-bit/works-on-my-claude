...
> 맨 위 기록의 **실측 필요(미확인) 목록**은 아래 v2.1.1 기록의 5번을 본다 — 5건 중 4건이 닫혔고,
> 남은 ②는 「보류」다. ③의 결론(골격에서 `allow` 4줄을 뺄 수 있다)은 아직 실행 전이며 `TASKS.md` 「할 일」에 있다.
...
### 5. 실측 필요 — **5건 중 4건이 닫혔다 (2026-08-10). 열려 있는 것은 ② 하나뿐이며 그것도 「보류」다.**
①~④ 는 v2.1.0 기록에서 그대로 이월된 것이고, ⑤ 가 이번에 새로 생겼다.
닫힌 것: ①(확인됨) · ③(확인됨 — 뺄 수 있음) · ④(불필요) · ⑤(확인됨 — 유지).
