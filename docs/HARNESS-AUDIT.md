...
> **감사 본체는 Claude Code 전체 버전이 지난 감사 기준과 다르면 돈다 — 완전히 같을 때만 건너뛴다.**
> (v3.7.0 에서 앞 두 자리 비교를 전체 버전 비교로 바꿨다. v3.0.0 에서 모델 트리거를 뺐다.
> 아래 옛 기록들이 「앞 두 자리가 같다」고 적은 것은 그때의 규칙이라 그대로 둔다.)
...
> - `open:project-skill-to-subagent` — 프로젝트 스킬(`.claude/skills/`)이 서브에이전트 스킬 목록에 실제로 뜨는지 (파생 자리: `docs/CHANGELOG.md` v3.9.0 항목)
> (출력 스타일 eject 확인은 v3.0.0 에서 대상 폐지로 닫혔다 — `docs/CHANGELOG.md` v3.0.0 항목.)
> (서브에이전트 캐시 수명 확인(파생 자리: v3.2.0 기록 4번①·6번)은 **2026-08-31 에 닫혔다** — 결론이 이미
> `.claude/rules/제약-공통.md` 「나중에 · 안 할 것」에 있어 한 사실이 두 곳이었다. 원문은 `docs/CHANGELOG.md`
> 「저장소 정리 — 트리거 없는 열린 확인 4건 이관」 절.)
> (하네스의 「Agent tool 을 시키지 마라」와 「적극 위임」 중 어느 쪽이 이기는지 보던 확인은 **2026-08-31 에 통과·닫혔다** — 「적극 위임」이 이겼다.
> 결론은 `docs/CHANGELOG.md` 「저장소 정리 — TASKS.md 「지금 하는 일」의 완료 기록 이관」 절. 파생 자리였던 v2.7.0 기록 6번②는 그 기록으로 끝났다.)
