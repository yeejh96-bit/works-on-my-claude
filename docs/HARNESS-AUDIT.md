...
> 맨 위 기록의 **실측 필요(미확인) 목록**은 아래 v2.1.1 기록의 5번을 본다 — 5건 전부 닫혔다(②는 「보류」로 닫음).
> ③의 결론(골격에서 `allow` 4줄을 뺄 수 있다)은 **v2.2.0 으로 실행됐다 (2026-08-10).**
>
> **다음 감사가 먼저 볼 것 — 열려 있는 2건 (2026-08-11 현재, 둘 다 「급하지 않음」으로 내림)**
> 아래 두 건은 **이미 `TASKS.md` 「할 일」에 `[ ]` 로 올라가 있다.** 다음 감사는 이것들을
> **새 「실측 필요」로 다시 만들지 말고**, 그 항목을 갱신하는 쪽으로 처리한다(같은 일이 두 줄로 늘어나는 것을 막는다).
> - **v2.2.0 갱신 모드가 옛 `allow` 4줄을 실제로 지우는지** — 아래 **5번③ 의 「이후 이력」** 참조.
>   `/womc update` 를 돌릴 일이 생겼을 때 겸사 확인한다. **단 갱신 전에 옛 4줄이 있던 폴더에서만 확인이 성립한다.**
> - **`subagentStatusLine`·`/statusline` 을 골격에 들일지** — 아래 **4번 절 「새로 챙길 만한 것」** 참조.
>   상태줄을 손볼 일이 생겼을 때 함께 판단한다.
...
> **이후 이력 — 2026-08-11: 위 2가지를 「급하지 않음」으로 내렸다. 열려 있지만 지금 붙잡지 않는다.**
> 지금 `.claude/statusline.js` 가 잘 돌고 있어 급할 이유가 없다. **상태줄을 손볼 일이 생겼을 때 함께 판단한다.**
> **이 건은 이미 `TASKS.md` 「할 일」에 `[ ]` 항목으로 있다** — 다음 감사는 새 「실측 필요」로 다시 만들지 말고 그 항목을 갱신한다.
> 확인 방법: 시험 폴더에서 `/statusline` 을 한 번 돌려 무엇이 생기는지 보고, 지금 `statusline.js` 가 보여 주는
> 5시간·주간 한도까지 나오는지 비교한다. 안 나오면 지금 것을 유지한다.

### 5. 실측 필요 — **5건 전부 닫혔다 (①③④⑤ 확인 완료 · ② 는 2026-08-11 「보류」로 닫음).**
①~④ 는 v2.1.0 기록에서 그대로 이월된 것이고, ⑤ 가 이번에 새로 생겼다.
닫힌 것: ①(확인됨) · ②(보류로 닫음) · ③(확인됨 — 뺄 수 있음 → v2.2.0 으로 실행) · ④(불필요) · ⑤(확인됨 — 유지).
**이 절에서 새로 파생된 열린 일 1건**은 ③의 「이후 이력」에 있다(갱신 모드 청소가 실제로 도는지 확인).

① ~~`outputStyle` 을 어떻게 고칠 것인가~~ → **확인됨 (위 2번①).** A안으로 해결됐다.
② ~~**`force-for-plugin: true` 가 이 판(2.1.226)에서 실제로 먹는지**~~ → **「보류」로 닫았다 (2026-08-11).**
   A안으로 해결됐으므로 실측할 이유가 없다. "프로젝트마다 `outputStyle` 을 박지 않게" 하고 싶어질 때
   **새 항목으로 다시 연다** — 열린 채로 두면 「할 일」을 위에서부터 집는 세션이 이걸 붙잡는다(`TASKS.md` 에서도 같은 이유로 `[x]` 로 닫았다).
   **다시 열 때의 확인 방법(보관)**: 스타일 frontmatter 에 넣고 플러그인 재설치 → 재시작 → `outputStyle` 이 **아예 없는** 폴더에서 말투가 나오는지 본다.
③ ~~**`permissions.allow` 의 PowerShell 4줄이 필요한지**~~ →
   ...
   **판정의 범위**: "실사용에서 프롬프트가 안 뜬다"까지가 확인된 것이다. 그것이 내장 자동 허용 때문인지
   ...
   >
   > **이후 이력 ② — 여기서 파생된 열린 일 1건 (2026-08-11 현재 「급하지 않음」).**
   > **그 청소 분기가 실제로 도는지는 아직 확인하지 못했다** — `check-sync.py` 로는 잡히지 않는다(스크립트는 문구 대조만 한다).
   > **이 건은 이미 `TASKS.md` 「할 일」에 `[ ]` 항목으로 있다** — 다음 감사는 새 「실측 필요」로 다시 만들지 말고 그 항목을 갱신한다.
   > 확인 방법: **갱신 전에 옛 4줄을 갖고 있던** 폴더에서 `/womc update` 를 돌린 뒤 `allow` 가 `[]` 가 됐는지 보고,
   > 사용자가 직접 넣은 항목은 남아 있는지 함께 본다. 플러그인 재설치 + **Claude Code 재시작** 뒤라야 의미가 있다.
   > **⚠ 이미 v2.2.0 이 반영된 폴더(이 저장소 포함)는 시험대가 못 된다** — 갱신 전후 모두 `[]` 이므로 `[]` 를 봤다는 것이 증거가 안 된다.
   > 그런 폴더에서 봤다면 항목을 **닫지 않는다.**
