...
- **버전을 올리는 작업이면 `PLAN.md`·`docs/CHANGELOG.md` 도 같은 작업에서 반드시 함께 갱신한다.** (버전 = `plugin.json`·`package.json` 등의 `version` 값.) 작은 변경이라 PLAN/TASKS 절차를 건너뛴 경우에도 예외가 없다 — 버전이 올라가면 기록도 올라간다. `PLAN.md` 의 버전 이력에 「무슨 버전 — 무엇을 바꿨는지」 한 줄을 더하고, `docs/CHANGELOG.md` 맨 위에 손댄 파일·남긴 것·확인 방법을 적는다. **끝난 것을 `TASKS.md` 에 쌓아 두지 않는다 — 하나라도 끝나면 바로 옮긴다.** 없으면 그때 만든다.
