...
- 반대로, 한 단계를 끝냈으면 **그 단계가 남긴 것(만들어진 파일 경로, 다음 단계가 쓸 함수·설정 이름)을 그것을 이어 쓰는 다음 항목의 「이어 쓸 것」 칸에 적고, 끝난 항목 자체는 `docs/CHANGELOG.md` 맨 위로 바로 옮긴다.** 이걸 빠뜨리면 다음 세션이 앞 단계의 결과물을 못 찾아 같은 것을 다시 만든다.
- **끝난 일이 남긴 제약(앞으로도 계속 지켜야 할 것)은 `TASKS.md` 에 적지 않는다.** `.claude/rules/제약-<영역>.md` 를 만들어 거기 적고, 맨 위 `paths` 에 그 제약이 걸리는 파일 경로를 적는다(맞는 파일이 없으면 새로 만든다). `TASKS.md` 에는 「지금 하는 일」·「할 일」만 둔다 — 할 일은 끝나면 사라지지만 제약은 사라지지 않아, 여기 쌓으면 목록을 열 때마다 지금과 관계없는 제약까지 전부 읽힌다. `paths` 로 두면 그 파일을 손댈 때만 켜진다.
- **`PLAN.md` 도 끝난 것을 쌓아 두지 않는다.** 한 단계가 끝나면 그 단계의 설계 경위를 `docs/CHANGELOG.md` 로 옮기고 「만드는 순서」의 그 줄에는 끝났다는 표시만 남긴다. 계획 전체가 닫히면(남은 단계 0개) 본문을 통째로 옮기고 「이 계획은 닫혔다 · 어디를 보면 되는지」 안내만 남긴다(20줄 이내). 앞으로도 지킬 제약·안 고른 길은 `PLAN.md` 가 아니라 `.claude/rules/제약-공통.md`(어디에나 걸리는 것)·`.claude/rules/제약-<영역>.md`(특정 파일에만 걸리는 것)에 적는다 — 계획은 끝나지만 제약은 안 끝나기 때문이다.
- **「⚠ 이 절은 옛 기록이다」 같은 배너를 붙여 절을 남기지 않는다.** 어떤 절을 「옛 기록」이라고 표시하고 싶어졌다면 그것은 그 절을 옮길 때가 됐다는 신호다 — 그 자리에서 `docs/CHANGELOG.md` 로 옮기고, 아직 유효한 문장만 뽑아 `.claude/rules/제약-*.md` 로 올린다. **한 사실은 한 곳에만 적는다** — 제약을 적기 전에 `.claude/rules/제약-*.md` 에 이미 있는지 먼저 보고, 있으면 다시 적지 않고 그쪽을 가리킨다.
- **버전을 올리는 작업이면 `docs/CHANGELOG.md` 를 같은 작업에서 반드시 함께 갱신한다.** (버전 = `plugin.json`·`package.json` 등의 `version` 값.) 작은 변경이라 PLAN/TASKS 절차를 건너뛴 경우에도 예외가 없다 — 버전이 올라가면 기록도 올라간다. `docs/CHANGELOG.md` 맨 위에 손댄 파일·남긴 것·확인 방법을 적고, 같은 파일 맨 아래 「버전 한 줄 요약 (색인)」에 「무슨 버전 — 무엇을 바꿨는지」 한 줄을 더한다. **끝난 것을 `TASKS.md` 에 쌓아 두지 않는다 — 하나라도 끝나면 바로 옮긴다.** 없으면 그때 만든다.
...
- **SPEC.md** — 이 프로젝트만의 상세 내용(스택, 만들 것, 기능 등). CLAUDE.md 끝에서
  ...
  (기존 프로젝트면 이미 있던 `CLAUDE.md`·설정에 womc 규칙을 덧붙여 병합하고, 다른 에이전트용 `AGENTS.md` 가 있으면 연결할지 물어보고, 코드 관례를 `.claude/rules/` 로 캡처하며, 구조 정리는 `plan-feature` 로 넘기도록 제안한다.)
- **.claude/settings.json** — 하네스(작업 환경) 기본값. 비밀키 파일(`.env` 와 그 변형)을 하위 폴더 것까지
  ...
  Claude Code 2.1.228 부터 이 `Read` 차단은 **파일을 새로 쓰거나 덮어쓰는 것까지 함께 막는다**(https://code.claude.com/docs/en/permissions). PowerShell 규칙은 별칭까지 잡아 주지만(`Remove-Item` 규칙 하나가 `rm`·`del`·`gci` 같은 별칭도 잡는다), Python·Node 스크립트처럼 프로그램이 대신 읽고 쓰는 것까지는 막지 못한다.
...
- **.claude/rules/** — 특정 작업에만 적용할 세부 규칙 자리. 여기 둔 규칙 파일은 **자동으로 인식된다** —
  ...
  앞으로 계속 지킬 제약(여기는 손대면 안 된다 같은 것)도 `TASKS.md` 가 아니라 여기 `제약-<영역>.md` 로 둔다 — 그래야 그 파일을 손댈 때만 읽히고, 할 일 목록이 제약으로 부풀지 않는다.
...
- **서브에이전트 4종** — 무거운 일을 별도 컨텍스트에서 처리한다. 메인(지휘자)이 조사·설계·구현·검증을
  ...
  `explore`(조사·빠른 haiku) · `plan`(설계·품질 위주 opus) · `implement`(구현·유일하게 파일을 직접 고침) · `verify`(동작 검증 — **매번 돌리는 단계가 아니라 확인을 부탁했거나 테스트 출력이 길 때만** 쓴다).
...
- `ask` 는 **되돌리기 어려운 명령만 골라 매번 다시 묻게** 만든다(`git push`·`git reset --hard`·브랜치 지우기·파일 지우기).
...
`## 나에 대해` 부터 `@SPEC.md` 까지 8개 절이 전부 들어간다 — 신규 프로젝트가 받는 규칙과 똑같이 맞추기 위해서다.
...
### 2-b) 기존 `AGENTS.md` 감지 (있을 때만)
`Glob` 로 **프로젝트 루트의 `AGENTS.md` 만** 본다. 하위 폴더의 `AGENTS.md` 는 무시한다 — 항상 로드되는 자리가 아니다. 루트에 없으면 이 소절 전체를 건너뛴다.
- 이 소절은 2) 와 달리 **「비-womc `CLAUDE.md` 일 때만」이라는 조건에 매이지 않는다** — 골격 그대로인 `CLAUDE.md` 든, 온보딩이 `womc:begin/end` 구획을 덧붙인 `CLAUDE.md` 든 똑같이 켜진다.
...
- 좋다고 하면 `Edit` 으로 `CLAUDE.md` 의 `## 프로젝트 상세` 절 안, **`@SPEC.md` 바로 다음 줄**에 `@AGENTS.md` 한 줄을 더한다. 파일을 통째로 다시 쓰지 않는다.
...
온보딩을 마치면 무엇을 했는지(SPEC 초안·병합·AGENTS 연결·캡처·구조 제안 중 실제로 한 것)를 한국어로 한 줄씩 알리고 "맞는지 검토하고 고쳐 주세요"로 맺는다.
...
1. 아래 불변 골격 파일들을 이 문서에 적힌 내용 그대로 **덮어쓴다**(없으면 새로 만든다).
   ...
   - `CLAUDE.md` — 세 경우로 나뉜다: ⓐ 첫 줄이 골격 그대로(`# 작업 규칙 (모든 프로젝트 공통 · 불변)`)면 **통째로 덮지 말고 절 단위로 병합한다**(바로 아래 절차). ⓑ **온보딩으로 사용자 파일에 병합된 형태(`womc:begin` 이 들어간 줄부터 `womc:end` 가 들어간 줄까지의 구획이 있는 경우)면 그 구획 안만 최신 내용으로 교체**하고 나머지 사용자 내용은 그대로 둔다. ⓒ 둘 다 아니면(사용자가 쓴 CLAUDE.md 인데 온보딩 병합을 안 거친 경우) **통째로 덮지 말고**, 위 '기존 프로젝트 온보딩'의 CLAUDE.md 병합 절차대로 `womc:begin/end` 구획을 맨 끝에 덧붙인다(사용자 내용 보존).
     ...
     ⓑ 로 구획 안을 교체할 때도 **구획 안에 있던 `@AGENTS.md` 줄은 이월한다** — 교체 전에 그 줄이 있었으면 교체 후에도 `## 프로젝트 상세` 절의 `@SPEC.md` **바로 다음 줄**에 다시 넣는다(아래 ⓐ 6단계와 같은 취지다). 온보딩에서 사용자가 이미 승낙한 줄이므로 **다시 묻지 않는다.**
     **ⓐ 의 절 단위 병합 절차.** 첫 줄이 골격 그대로여도 **파일 중간에 사용자가 직접 써 넣은 절이 있을 수 있다** — 실측 사례: 골격의 「절차 지키기」 절과 「프로젝트 상세」 절 사이에 그 프로젝트만의 절대 원칙 절(개인정보를 LLM 에 보내지 말 것 등)이 들어 있었다. 통째로 덮으면 그게 전부 사라진다. 그래서 이렇게 한다:
     ...
     6. **덮기 전에 있던 `@AGENTS.md` 줄은 이월한다.** 기존 파일의 `## 프로젝트 상세` 절에 그 줄이 있었으면, 교체한 뒤에도 `@SPEC.md` **바로 다음 줄**에 다시 넣는다. 온보딩('기존 `AGENTS.md` 감지')에서 사용자가 이미 승낙한 표시이므로 **다시 묻지 않는다.** (`## 프로젝트 상세` 는 위 「골격 절 제목 8개」에 들어 있어 통째로 교체된다 — 이월하지 않으면 `/womc update` 한 번에 `AGENTS.md` 연결이 조용히 사라지고 사용자는 알아챌 방법이 없다.)
     ...
     **`@AGENTS.md` 줄을 이월했는지 / 제안만 했는지 / 루트에 `AGENTS.md` 가 없어 해당 없음이었는지를 완료 보고에 한 줄 적는다.**
...
7. **하네스 감사 (자동).** 위 1~6번으로 골격 갱신을 마쳤으면 곧바로 이어서 한다 — 사용자가 따로 부르지 않아도 된다.
   - `docs/HARNESS-AUDIT.md` 의 맨 위 기록에 적힌 「마지막 감사 기준 버전」·「마지막 감사 기준 모델」을 지금 `claude --version` 과 **지금 세션의 모델 이름**에 비교한다. 파일이 없으면 이번이 첫 감사다 — 그냥 진행한다.
     ...
     **모델 비교는 이름이 같은지만 본다.** 기록에 기준 모델이 없으면(옛 기록) **다른 것으로 친다** — 한 번은 모델 기준으로 훑고 지나가게 하려는 것이다.
   - **버전 앞 두 자리도 같고 모델도 같을 때만 감사 본체를 건너뛴다** — 한 줄로 알리되 두 경우를 구분한다: 버전이 완전히 같으면 "지난 감사 이후 Claude Code 도 모델도 안 바뀌었음", 패치만 다르면 "패치만 올랐음(2.1.226 → 2.1.228) — 버그 수정 위주라 신기능 조사는 건너뜀. 모델도 그대로임". 이어서 **"그래도 지금 조사할까요?" 한 줄을 덧붙인다** — 사용자가 좋다고 하면 아래 「다르면」과 똑같이 `harness-audit` 스킬을 그 자리에서 실행한다.
   ...
   - **버전 앞 두 자리가 다르거나 모델이 다르면** `harness-audit` 스킬을 그 자리에서 실행한다. 이때 조사 구간은 패치까지 포함한 **「마지막 감사 기준 버전 ~ 현재 버전」 전체**이고, 모델이 바뀌었으면 **「지난 기준 모델 → 현재 모델」**도 함께다. (스킬 절차를 여기 베껴 적지 않는다 — 절차의 진실은 `skills/harness-audit/SKILL.md` 한 곳에만 둔다.)
